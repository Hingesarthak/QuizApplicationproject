...
public class resulteasyset5 extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_resulteasyset5);
        ...
        if ("Yellow".equalsIgnoreCase(selectedOption1)) score++;
        if ("366".equalsIgnoreCase(selectedOption2)) score++;
        if ("Oxygen".equalsIgnoreCase(selectedOption3)) score++;
        if ("Square".equalsIgnoreCase(selectedOption4)) score++;
        if ("Tomato".equalsIgnoreCase(selectedOption5)) score++;
        if ("Ice".equalsIgnoreCase(selectedOption6)) score++;
        if ("Parrot".equalsIgnoreCase(selectedOption7)) score++;
        if ("Milk".equalsIgnoreCase(selectedOption8)) score++;
        if ("Photosynthesis".equalsIgnoreCase(selectedOption9)) score++;
        if ("0°C".equalsIgnoreCase(selectedOption10)) score++;


        Toast.makeText(resulteasyset5.this, "You've Scored : " + score, Toast.LENGTH_LONG).show();
